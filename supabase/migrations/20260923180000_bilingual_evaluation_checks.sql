-- Keep the database's strict cloud payload validator aligned with the
-- canonical evaluator contract: bilingual learner-safe details plus a stable
-- internal diagnostic code for every rubric check.
...
begin
    ...
    for item in select value from jsonb_array_elements(p_value -> 'checks') loop
        ...
        perform public._cloud_json_string(item, 'details_ar', p_label || '.check', true);
        perform public._cloud_json_string(item, 'details_en', p_label || '.check', true);
