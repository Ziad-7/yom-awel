...
def test_artifact_content_type_migration_expands_backfills_and_keeps_rollback_compatibility() -> None:
    migration = (
        Path(__file__).parents[4]
        / "supabase/migrations/20260924150000_artifact_content_type.sql"
    ...
    assert "when lower(filename) like '%.xlsx' then 'application/vnd.openxmlformats-officedocument.spreadsheetml.sheet'" in migration
