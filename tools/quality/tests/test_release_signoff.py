...
def test_release_signoff_is_valid() -> None:
    ...
    for doc in ["product-acceptance.md", "known-limitations.md", "submission-checklist.md"]:
...
def test_go_decision_has_all_passing_gates_and_five_member_reviews() -> None:
    ...
    if decision == "go":
        ...
        for gate_name, gate_info in gates.items():
            assert gate_info.get("status") == "passed", f"Quality gate '{gate_name}' is not passed"
        ...
        for m_idx in range(1, 6):
            ...
            assert reviews[member_key].get("status") == "approved", f"Approval for '{member_key}' not approved"
